...
$colname_Rechome = "-1";
if (isset($_GET['d_id'])) {
  $colname_Rechome = $_GET['d_id'];
}
mysql_select_db($database_connect2data, $connect2data);
$query_Rechome = sprintf("SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_id = %s", GetSQLValueString($colname_Rechome, "int"));
...
//banner
$query_RecImageBanner = sprintf("SELECT * FROM file_set WHERE file_d_id = %s AND file_type = 'homeBanner'", GetSQLValueString($colname_RecImage, "int"));
...
$totalRows_RechomeC = mysql_num_rows($RechomeC);

mysql_select_db($database_connect2data, $connect2data);
$query_RechomeY = "SELECT * FROM class_set WHERE c_parent = 'homeY' AND c_active='1' ORDER BY c_sort ASC, c_id DESC";
$RechomeY = mysql_query($query_RechomeY, $connect2data) or die(mysql_error());
$row_RechomeY = mysql_fetch_assoc($RechomeY);
$totalRows_RechomeY = mysql_num_rows($RechomeY);
...
$G_selected1 = '';
if (isset($_SESSION['selected_homeC'])){
	$G_selected1 = $_SESSION['selected_homeC'] = $row_Rechome['d_class2'];
                   ...
                   <tr>
                     ...
                     <td>
                       <select name="d_class2" id="d_class2">
                         ...
                         do {
                          ?>
                          <option value="<?php echo $row_RechomeC['c_id']?>" <?php if (!(strcmp($row_RechomeC['c_id'], $row_Rechome['d_class2']))) {echo "selected=\"selected\"";} ?>><?php echo $row_RechomeC['c_title']?></option>
                          <?php
                     ...
                     <tr>
                       ...
                       <td><input name="d_title" type="text" class="table_data" id="d_title" size="80" value="<?php echo $row_Rechome['d_title']; ?>" /></td>
                       <td bgcolor="#e5ecf6">&nbsp;</td>
                     </tr>

                     <tr>
                       <td align="center" bgcolor="#e5ecf6" class="table_col_title">內容</td>
                       <td><textarea name="d_content" cols="50" rows="30" class="table_data tiny" id="d_content"><?php echo $row_Rechome['d_content']; ?></textarea></td>
                     ...
                     <tr>
                      ...
                      <td><input name="d_date" type="text" class="table_data" id="d_date" value="<?php echo $row_Rechome['d_date']; ?>" size="50" /></td>
                      <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                    </tr>
                    <tr>
                     ...
                     <td><label>
                       <select name="d_active" class="table_data" id="d_active">
                         <option value="0" <?php if (!(strcmp(0, $row_Rechome['d_active']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                         <option value="1" <?php if (!(strcmp(1, $row_Rechome['d_active']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                       </select>
                     </label></td>
                     <td bgcolor="#e5ecf6">&nbsp;</td>
                   </tr>
                   ...
                   <tr>
                     ...
                     <td><label>
                       <select name="d_data1" class="table_data" id="d_data1">
                         <option value="0" <?php if (!(strcmp(0, $row_Rechome['d_data1']))) {echo "selected=\"selected\"";} ?>>不公佈</option>
                         <option value="1" <?php if (!(strcmp(1, $row_Rechome['d_data1']))) {echo "selected=\"selected\"";} ?>>公佈</option>
                       </select>
                     </label></td>
                     <td bgcolor="#e5ecf6">&nbsp;</td>
                   </tr>
                   ...
                   <!-- <tr>
                    ...
                    <td><?php do { ?>
                      <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                          <td align="center"><a href="image_edit.php?file_id=<?php echo $row_RecImageBanner['file_id'].'&type=homeBanner'; ?>" class="fancyboxEdit" title="修改圖片"><img src="image/media_edit.gif" width="16" height="16" title="修改圖片"/></a><a href="image_del.php?file_id=<?php echo $row_RecImageBanner['file_id'].'&type=homeBanner'; ?>" class="fancyboxEdit" title="刪除圖片"><img src="image/media_delete.gif" width="16" height="16" title="刪除圖片"/></a></td>
                          <td align="center">&nbsp;</td>
                        </tr>
                      </table>
                    ...
                    <!-- <tr>
                      ...
                      <td>
                       ...
                       <td bgcolor="#e5ecf6" class="table_col_title">
                        <p class="red_letter">*<?php echo $imagesSize['homeBanner']['note'];?></p></td>
                      </tr> -->
                      <?php } // Show if recordset not empty ?>

                      <?php if ($totalRows_RecImage > 0) { // Show if recordset not empty ?>
                      <tr>
                        <td align="center" bgcolor="#e5ecf6" class="table_col_title">目前封面圖片(有封面才可顯示在最新活動)<a name="imageEdit" id="imageEdit"></a></td>
                        <td><?php do { ?>
                          <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
...
<input name="d_id" type="hidden" id="d_id" value="<?php echo $row_Rechome['d_id']; ?>" />
<input name="d_sort" type="hidden" id="d_sort" value="<?php echo $row_Rechome['d_sort']; ?>" />
</form>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 ...
 $image_result=image_process($_FILES['image'], $_REQUEST['image_title'], "home","add", $imagesSize[$_SESSION['nowMenu']]['IW'], $imagesSize[$_SESSION['nowMenu']]['IH']);
 ...
 $image_result=image_process($_FILES['imageBanner'], $_REQUEST['imageBanner_title'], "homeBanner","add", $imagesSize['homeBanner']['IW'], $imagesSize['homeBanner']['IH']);
    //echo count($image_result);
    //echo $image_result[0][0];
 ...
 for($j=1;$j<count($image_result);$j++)
 {
   $insertSQL = sprintf("INSERT INTO file_set (file_name, file_link1, file_link2, file_link3, file_type, file_d_id, file_title, file_show_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
     ...
     GetSQLValueString($image_result[$j][2], "text"),
     GetSQLValueString($image_result[$j][3], "text"),
     GetSQLValueString("homeBanner", "text"),
     GetSQLValueString($d_id, "int"),
     GetSQLValueString($image_result[$j][4], "text"),
     GetSQLValueString($image_result[$j][5], "int"));
 ...
 if($ifFile){
    //----------插入檔案資料到資料庫begin(須放入插入主資料後)----------
      ...
      $file_result=file_process("homeFile","add");
  ...
  //----------插入檔案資料到資料庫end----------
    ...
    $_SESSION['original_selected']=$_SESSION['selected_homeC'];

  //$updateGoTo = "home_list.php?selected1=".$class2."&selected2=".$class3."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$_POST['d_id']."&totalRows_Rechome=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
    $updateGoTo = "home_list.php?selected1=".$d_class2."&changeSort=1&change_num=".$_POST['d_sort']."&now_d_id=".$d_id."&totalRows=".$_SESSION['totalRows']."&pageNum=".$_SESSION["ToPage"];
